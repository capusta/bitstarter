...
module.exports = function (passport) {
    ...
    passport.serializeUser(function(u, done) {
        done(null,  u.dataValues.username.toLowerCase());
    });

    passport.deserializeUser(function(uname, done) {
        global.db.User.find({where: {username: uname.toLowerCase()}}).success(function(u) {
            if(u){
                done(null, u);
            } else{
    ...
    function(uname, pword, done) {
        global.db.User.find({where: {username: uname.toLowerCase()}}).success(function(user_instance) {
